...
# App:        HEX generated by tools/sign_image.py (contains all header patches
#             and Ed25519 signature).  The .elf has zero header fields and must
#             NOT be flashed directly.
$imageMap = @{
...
foreach ($t in $targets) {
    ...
    if (-not (Test-Path $img)) {
        if ($t -eq 'app') {
            Write-Error (
                "App image not found: $img`n" +
                "Build and sign the app first:`n" +
                "  1. .\build.ps1 -Config $Config`n" +
                "  2. python tools\sign_image.py build\app\$Config\App.bin"
            )
        } else {
